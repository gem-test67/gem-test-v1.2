...
const SpotifyControls: React.FC<SpotifyControlsProps> = ({ onSendCommand, isLoading }) => {
  const [songName, setSongName] = useState('');

  const handlePlay = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (songName.trim()) {
      onSendCommand(`Play the song "${songName}" on Spotify`);
    } else {
      onSendCommand('Play Spotify');
    }
    // setSongName(''); // Optional: clear input after sending
  };

  const handlePause = () => {
    onSendCommand('Pause Spotify');
  };

  const handleNext = () => {
    onSendCommand('Play next track on Spotify');
  };

  return (
    <div className="p-4 bg-gray-800/50 rounded-lg mb-4">
      <h3 className="text-sm font-bold text-purple-300 mb-3 tracking-wider">SPOTIFY</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <form onSubmit={handlePlay} className="flex items-center gap-2 p-3 bg-gray-700/50 rounded-lg">
          <input
            ...
            disabled={isLoading}
            className="flex-1 min-w-0 p-1.5 bg-gray-800 border border-gray-600 rounded-md disabled:opacity-50 focus:ring-2 focus:ring-purple-500 focus:outline-none"
